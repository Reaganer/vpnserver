...
// Package sha implements the SHA-0 hash algorithm as defined in FIPSPUB 180.
//
// SHA-0 is cryptographically broken and should not be used for secure
// applications.
package sha
...
const SHA0 crypto.Hash = 100

func init() {
	//crypto.RegisterHash(SHA0, New)
}

// The size of a SHA-0 checksum in bytes.
const Size = 20

// The blocksize of SHA-0 in bytes.
const BlockSize = 64
...
func (d *digest) UnmarshalBinary(b []byte) error {
	if len(b) < len(magic) || string(b[:len(magic)]) != magic {
		return errors.New("my/sha: invalid hash state identifier")
	}
	if len(b) != marshaledSize {
		return errors.New("my/sha: invalid hash state size")
	}
	b = b[len(magic):]
...
// Sum returns the SHA-0 checksum of the data.
